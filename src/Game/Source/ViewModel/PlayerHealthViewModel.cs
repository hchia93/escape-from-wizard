        ...
        public void DrawWidget(SpriteBatch spriteBatch)
        {
            if (m_SourceSpriteSheet != null)
            {
                for (int i = 0; i < m_MaxHP; i++)
                {
                    ...
                    if (i < m_HP)
                    {
                        spriteBatch.Draw(m_SpriteSheetTexture, heartPosition, m_FilledHeartRectangle, Color.White);
                    }
                    else
                    {
                        spriteBatch.Draw(m_SpriteSheetTexture, heartPosition, m_EmptyHeartRectangle, Color.White);
                    }
                }
            }
        }
