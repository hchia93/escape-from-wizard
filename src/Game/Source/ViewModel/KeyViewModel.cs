...
using System;
using Game.Source.Interface;
using Game.Source.TileEngine;
using Game.Source.GameObject.Static;

namespace Game.Source.ViewModel
{
    public class KeyViewModel : IWidget
    {
        ...
        public void DrawWidget(SpriteBatch spriteBatch)
        {
            if (m_SourceSpriteSheet != null)
            {
                if (m_IsLooted)
                {
                    spriteBatch.Draw(m_SpriteSheetTexture, m_WidgetPosition, m_PostLootedRectangle, Color.White);  
                }
                else
                {
                    spriteBatch.Draw(m_SpriteSheetTexture, m_WidgetPosition, m_PreLootedRectangle, Color.White);
                }
            }
        }
    }
